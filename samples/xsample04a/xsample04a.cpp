...
public:
    ...
    void BuildParallel(const vector<char>& sequence)
    {
        struct Func
        {
            void operator() (bm::bvector<>* target, const vector<char>& sequence, char base)
            {
                bm::bvector<>::bulk_insert_iterator iit(*target, bm::BM_SORTED);
                for (size_t i = 0; i < sequence.size(); ++i)
                {
                    if (sequence[i] == base)
                        iit = unsigned(i);
        ...
        // process DNA letters in parallel
        //
        std::future<void> f1 = std::async(std::launch::async, 
                                  Func(), &m_FPrintBV[eA], sequence, 'A');
        std::future<void> f2 = std::async(std::launch::async,
                                  Func(), &m_FPrintBV[eT], sequence, 'T');
        std::future<void> f3 = std::async(std::launch::async,
                                  Func(), &m_FPrintBV[eG], sequence, 'G');
        std::future<void> f4 = std::async(std::launch::async,
                                  Func(), &m_FPrintBV[eC], sequence, 'C');

        {
            Func f;
            f(&m_FPrintBV[eN], sequence, 'N');
        }
