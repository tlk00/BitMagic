            ...
            if (/*(delta_acc > drange_gain_cutoff) &&*/ /*(len > 5)*/ true) // drange gain cut-off
            {
                ...
                if (ex0_first)
                {
                    if (ex0_cnt)
                    {
                        ...
                        bout.encode_array(&ex0_arr[0], &gap_recalc_block[0], ex0_cnt, false, end_fl);
                        if (ex1_cnt)
                            bout.encode_array(&ex1_arr[0], &gap_recalc_block[0], ex1_cnt, true, true);
                    }
                }
                else // ex1 first
                {
                    bool end_fl = (ex0_cnt == 0);
                    bout.encode_array(&ex1_arr[0], &gap_recalc_block[0], ex1_cnt, true, end_fl);
                    if (ex0_cnt)
                        bout.encode_array(&ex0_arr[0], &gap_recalc_block[0], ex0_cnt, false, true);
                }
                bout.flush();
#ifdef BM_DBG_SERIAL
                                     ...
                                     bm::encoder& enc) BMNOEXCEPT
{
    ...
    {
        if ((compression_level_ > 3)) // Use Elias Gamma encoding
        {
            {
                bit_out_type bout(enc);
                ...
                bm::for_each_dgap(gap_block, gamma);
            }
            ...
            if (gamma_size > (len * sizeof(gap_word_t)))
            {
                enc.set_pos(enc_pos0);
            }
            else
    ...
    unsigned enc_size = (unsigned)(enc.get_pos() - enc_pos0); (void) enc_size;
    compression_stat_[bm::set_block_gap]++;
}
...
//    const float bie_bits_per_int = compression_level_ < 6 ? 3.75f : 2.5f;
    ...
    if ((compression_level_ >= 6) && bic_drange_) // DRange applicable tweaks
    {
        ...
        case bm::set_block_arr_bienc:
            ...
            if (bit_stat_.gc < bm::gap_max_safe_len)
            {
    //BM_ASSERT(bit_stat_.gc >= bit_stat_.bc);
                ...
                if (r < 0.17)
                    model = bm::set_block_gap_bienc;
    //            const char* c = bit_stat_.ibc < bit_stat_.ibc ? "i" : "d";
    //            std::cout << c;
    //            std::cout << " [gc=" << bit_stat_.gc << " bc=" << bit_stat_.bc << "] r=" << r << "\n";
            }
        ...
        case bm::set_block_bitgap_bienc:
            if (bit_stat_.gc > bm::gap_max_safe_len)
            {
                if (bit_stat_.bc <= bit_stat_.ibc)
                {
                    ...
                    float r = float(diff) / float(bit_stat_.bc);
    //std::cout << r << std::endl;
                    //std::cout << " +[gc=" << bit_stat_.gc << " bc=" << bit_stat_.bc << " ibc=" << bit_stat_.ibc << "] r=" << r << "\n";
                    model = bm::set_block_arr_bienc;
                }
                else // inverted
                {
                    int diff = int(bit_stat_.gc) - int(bit_stat_.ibc);
                    if (diff < 0)
                        diff = -diff;
                    float r = float(diff) / float(bit_stat_.ibc);
                    //std::cout << " -[gc=" << bit_stat_.gc << " ibc=" << bit_stat_.ibc << "] r=" << r << "\n";
                                                ...
                                                bm::encoder&      enc) BMNOEXCEPT
{
    ...
    {
/*
        bool inverted = (bit_stat_.ibc < bit_stat_.bc);
        interpolated_arr_bit_block(block, enc, inverted);
*/
                                              ...
                                              bool inverted)
{
...

    {
        ...
        if (r_cnt)
        {
            ...
            bout.encode_array(arr_r, arr_tmp, r_cnt, !inverted, false);
            auto p2 = enc.get_pos();
...
/*
        ...
        {
        bit_out_type bout(enc);
        bout.encode_array(&bit_idx_arr_[0],
                         (bm::gap_word_t*) gap_recalc_tmp_block0_,
                         arr_len, !inverted, false);
        bout.flush();
        }
        encoder::position_type enc_pos1 = enc.get_pos();
        unsigned enc_size1 = (unsigned)(enc_pos1 - enc_pos0)+1;

        enc.set_pos(enc_pos0); // rollback stream unconditionlly
*/
        ...
        if (r)
        {
            ...
            return;
/*
            encoder::position_type enc_pos2 = enc.get_pos();
            unsigned enc_size2 = (unsigned)(enc_pos2 - enc_pos0);
            if (enc_size2 <= enc_size1)
            {
dr_cnt++;
                return; // good job BIC-DR
            }
            else
            {
no_dr_cnt++;
if (no_dr_cnt % 1000 == 0)
{
    (void)no_dr_cnt;
                ...
                enc.set_pos(enc_pos0); // BIC-DR failed :( - rollback stream
            }
*/
        }
        else
...
*/
            ...
            if (bool no_gap_EOC = !(h3f & bm::h3f_ex_arr_ex_EOC); no_gap_EOC)
            {
                ...
                h3f = bin.decode_array(arr_r, &r_cnt);
                BM_ASSERT((h3f & bm::h3f_ex_arr_1) ||
                          (block_type == bm::set_block_arr_bienc_inv_v3));

                h3f = bin.decode_array(arr_rl, &s_cnt, r_cnt);
    ...
    case set_block_gap_bienc_v3:
        {
            ...
            if (head_v3 & bm::h3f_exceptions)
            {
                ...
                unsigned h3f = bin.decode_array(&ex0_arr[0], &ex0_cnt);
                ...
                {
                    h3f = bin.decode_array(&ex0_arr[0], &ex0_cnt);
                                          ...
                                          bm::word_t*          /*temp_block*/)
{
    ...
    do
    {
        ...
        case set_block_arrgap_bienc_v2:
        case set_block_arrgap_bienc_inv_v2:
//        case set_block_arrgap_bienc_v3:
//        case set_block_arrgap_bienc_inv_v3:
        case set_block_gap_bienc_v3:
//std::cout << "GAP des: nb_i=" << nb_i << std::endl;
