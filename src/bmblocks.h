...
public:
    ...
    public:
        ...
        block_count_arr_func(const blocks_manager& bm, unsigned* arr) BMNOEXEPT
            : block_count_base(bm), arr_(arr), last_idx_(0) 
        {
            arr_[0] = 0;
        }

        void operator()(const bm::word_t* block, id_type idx) BMNOEXEPT
        {
            while (++last_idx_ < idx)
            {
                arr_[last_idx_] = 0;
        ...
        id_type last_block() const BMNOEXEPT { return last_idx_; }
        void on_non_empty_top(unsigned) BMNOEXEPT {}
    ...
    public:
        ...
        block_count_change_func(const blocks_manager& bm) BMNOEXEPT
            : bm_func_base_const(bm),
                count_(0),
        ...
        void operator()(const bm::word_t* block, block_idx_type idx) BMNOEXEPT
        {
    ...
    public:
        ...
        block_any_func(const blocks_manager& bm) BMNOEXEPT
            : bm_func_base_const(bm) 
        {}

        bool operator()
                (const bm::word_t* block, block_idx_type /*idx*/) BMNOEXEPT
        {
...
public:
    ...
    */
    unsigned compute_top_block_size(id_type bits_to_store) const BMNOEXEPT
    {
    ...
    */
    const bm::word_t*
    get_block(block_idx_type nb, int* no_more_blocks) const BMNOEXEPT
    {
    ...
    */
    block_idx_type
    find_next_nz_block(block_idx_type nb, bool deep_scan=true) const BMNOEXEPT
    {
        if (is_init())
        {
            unsigned i,j;
    ...
    /**
        ...
        \return block adress or NULL if not yet allocated
    */
    const bm::word_t* get_block(unsigned i, unsigned j) const BMNOEXEPT
    {
    ...
    /**
        ...
        \return block adress or NULL if not yet allocated
    */
    const bm::word_t* get_block_ptr(unsigned i, unsigned j) const BMNOEXEPT
    {
        if (!top_blocks_ || i >= top_block_size_) return 0;
    ...
    /**
        ...
        \return block adress or NULL if not yet allocated
    */
    bm::word_t* get_block_ptr(unsigned i, unsigned j) BMNOEXEPT
    {
    ...
    /**
        ...
        \return block adress or NULL if not yet allocated
    */
    const bm::word_t* const * get_topblock(unsigned i) const BMNOEXEPT
    {
    ...
    */
    bm::word_t*** top_blocks_root() const BMNOEXEPT
    {
        blocks_manager* bm = 
    ...
    */
    BMFORCEINLINE
    void set_block_ptr(unsigned i, unsigned j, bm::word_t* block) BMNOEXEPT
    {
        BM_ASSERT(is_init());
        BM_ASSERT(i < top_block_size_);
    ...
    */
    static
    bm::id_t block_bitcount(const bm::word_t* block) BMNOEXEPT
    {
        BM_ASSERT(block);
        id_t count;
        if (BM_IS_GAP(block))
    ...
    */
    bool is_subblock_null(unsigned nsub) const BMNOEXEPT
    {
        BM_ASSERT(top_blocks_);
    ...
    bm::word_t*** top_blocks_root() BMNOEXEPT
    {
        return top_blocks_;
    }
    ...
    */
    const gap_word_t* glen() const BMNOEXEPT
    {
        return glevel_len_;
    }
    ...
    */
    unsigned glen(unsigned level) const BMNOEXEPT
    {
    ...
    */
    unsigned top_block_size() const BMNOEXEPT
    {
    ...
    */
    allocator_type& get_allocator() BMNOEXEPT { return alloc_; }

    /** \brief Returns allocator
    */
    allocator_type get_allocator() const BMNOEXEPT { return alloc_; }
    ...
    */
    ...
    unsigned find_real_top_blocks() const BMNOEXEPT
    {
        unsigned cnt = 0;
        unsigned top_blocks = top_block_size();
        // TODO: SIMD
    ...
    unsigned find_max_top_blocks() const BMNOEXEPT
    {
        unsigned top_blocks = top_block_size();
        if (!top_blocks)
            return 0;
    ...
    */
    size_t calc_serialization_null_full() const BMNOEXEPT
    {
...
{
public:
    bit_block_guard(BlocksManager& bman, bm::word_t* blk=0) BMNOEXEPT
        : bman_(bman), 
          block_(blk)
    {}
    ~bit_block_guard()
    ...
    bm::word_t* get() BMNOEXEPT { return block_; }
